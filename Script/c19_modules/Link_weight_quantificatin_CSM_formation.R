...
graph.Raw <- read.table("Human_Node.txt",sep = '\t',header = TRUE)
...
loc_node_param <- read.table("Location_Node.txt",sep = '\t',header = TRUE)
...
write.table(cs_matrix,"contact_matrix.txt",sep = '\t',row.names = TRUE, col.names = TRUE,quote = F)
